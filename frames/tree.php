...
<head>
	...
	<script type="text/javascript">
		...
		function hideMenu() {
			if(_rc) return false;
			document.getElementById('mx_contextmenu').style.visibility = 'hidden';
		}
		...
		function expandTree() {
			...
			new Ajax('index.php?a=1&f=nodes&indent=1&parent=0&expandAll=1', {
				method: 'get',
				onComplete: rpcLoadData
			}).request();
		}
		...
		function collapseTree() {
			rpcNode = document.getElementById('treeRoot');
			new Ajax('index.php?a=1&f=nodes&indent=1&parent=0&expandAll=0', {
				method: 'get',
				onComplete: rpcLoadData
			}).request();
		}
		...
		function restoreTree() {
			rpcNode = document.getElementById('treeRoot');
			new Ajax('index.php?a=1&f=nodes&indent=1&parent=0&expandAll=2', {
				method: 'get',
				onComplete: rpcLoadData
			}).request();
		}
		...
		function setSelected(elSel) {
			jQuery('.treeNodeSelected', jQuery('#treeRoot')).removeClass('treeNodeSelected');
			jQuery(elSel).addClass('treeNodeSelected');
		}

		function setHoverClass(el, dir) {
			if(dir == 1) {
				jQuery(el).addClass('treeNodeHover');
			} else {
				jQuery(el).removeClass('treeNodeHover');
			}
		}
			...
			if(ca == "open" || ca == "") {
				if(id == 0) {
					...
					if(e.shiftKey) {
						...
						parent.mainMenu.reloadtree(); // Show updated locks as &r=1 will not work in popup
					} else {
			...
			<td>
				<table cellpadding="0" cellspacing="0" border="0">
					<tr>
						...
						<?php } ?>
						<td>
							<a href="#" class="treeButton" id="Button4" onClick="top.mainMenu.reloadtree();" title="<?php echo $_lang['refresh_tree']; ?>"><?php echo $_style['refresh_tree']; ?></a>
						</td>
						<td>
